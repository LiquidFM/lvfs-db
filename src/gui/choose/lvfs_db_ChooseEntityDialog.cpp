...
#include "lvfs_db_ChooseEntityDialog.h"
...
public:
    ...
    {
        return Core::Qt::SortFilterModel::compareFileNames(t1.name().c_str(), t2.name().c_str());
    }
};
...
{
public:
    ChoosePropertyDialog(const QString &title, const Interface::Adaptor<IStorage> &container, const LiquidDb::Storage::Entities &entities, QWidget *parent = 0) :
...
ChooseEntityDialog::ChooseEntityDialog(const QString &title, const Interface::Adaptor<IStorage> &container, const LiquidDb::Storage::Entities &entities, QWidget *parent) :
    QDialog(parent),
...
Entity ChooseEntityDialog::chooseFile(const Interface::Adaptor<IStorage> &container, QWidget *parent)
{
    LiquidDb::Storage::Entities entities;

    for (auto i : container->entities())
        if (container->schema(i.second) == IStorage::Path)
    ...
    if (entities.empty())
        QMessageBox::information(parent,
                                 tr("Entity not found"),
                                 tr("There is no entities with property representing file system path."));
    else
        if (entities.size() == 1)
...
Entity ChooseEntityDialog::chooseProperty(const Interface::Adaptor<IStorage> &container, const Entity &entity, ::EFC::String &propertyName, QWidget *parent)
{
